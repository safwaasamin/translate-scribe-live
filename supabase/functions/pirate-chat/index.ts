...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Pirate chat request with messages:", messages.length);
    ...
    if (!response.ok) {
      if (response.status === 429) {
        return new Response(
          JSON.stringify({ error: "Rate limits exceeded, please try again later, ye landlubber!" }),
          { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      if (response.status === 402) {
        return new Response(
          JSON.stringify({ error: "Payment required! The treasure chest be empty!" }),
    ...
    console.error("Pirate chat error:", error);
    return new Response(
